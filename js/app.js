...
function renderPoster() {
    ...
    var spotItems = SITES.map(function (site) {
        return '<div class="poster-spot-card" data-spot="' + site.id + '" style="background-image:url(' + site.img + ')">' +
            '<div class="poster-spot-name">' + site.name + '</div>' +
        '</div>';
    }).join("");
    ...
    return el(
        '<div class="view poster" id="view-poster">' +
            ...
            '<div class="poster-section">' +
                '<div class="style-section-title">选择景点</div>' +
                '<div class="poster-spot-grid">' + spotItems + '</div>' +
            '</div>' +
            '<div class="poster-section">' +
                '<div class="style-section-title">选择风格</div>' +
            ...
            '<div class="generate-section">' +
                ...
                '<div class="generate-note" id="poster-note">选择景点和风格后即可生成专属海报</div>' +
            '</div>' +
        '</div>'
    );
}
...
function updatePosterButton() {
    ...
    if (gen) {
        gen.disabled = !(posterState.selectedSpot && posterState.selectedStyle && !posterState.isGenerating);
    }
    if (note) {
        if (!posterState.selectedSpot) note.textContent = "请在上方选择一个仓城景点";
        else if (!posterState.selectedStyle) note.textContent = "请选择一种风格";
...
function generatePoster() {
    ...
    if (!spot || !cfg) {
        handlePosterError("请先选择景点和风格");
        posterState.isGenerating = false;
        return;
    }

    loadImageAsBase64(spot.img).then(function (base64) {
        return compressImage(base64, 512, 0.8);
...
function composePoster(imageUrl, spot) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        img.onload = function () {
            ...
            // 顶部渐变遮罩
            var tg = ctx.createLinearGradient(0, 0, 0, H * 0.45);
            ...
            ctx.font = "bold 120px 'PingFang SC','Hiragino Sans GB','Noto Serif SC',serif";
            ctx.textAlign = "center";
            ctx.shadowColor = "rgba(0,0,0,0.3)";
            ctx.shadowBlur = 8;
            ctx.fillText(spot.name, W / 2, H * 0.22);
            ctx.shadowBlur = 0;

            // 底部渐变
            var bg = ctx.createLinearGradient(0, H * 0.6, 0, H);
            bg.addColorStop(0, "rgba(0,0,0,0)");
            bg.addColorStop(0.4, "rgba(0,0,0,0.3)");
            bg.addColorStop(1, "rgba(0,0,0,0.7)");
            ctx.fillStyle = bg;
            ctx.fillRect(0, H * 0.6, W, H * 0.4);

            // 诗意标语
            var tagline = POSTER_TAGLINES[spot.id] || "";
            var lines = tagline.split("\n");
            ctx.fillStyle = "rgba(255,255,255,0.9)";
            ctx.font = "56px 'PingFang SC','Hiragino Sans GB','Noto Serif SC',serif";
            var lineY = H * 0.77;
            for (var i = 0; i < lines.length; i++) {
                ctx.fillText(lines[i], W / 2, lineY + i * 72);
            }

            // 品牌标识
            ctx.fillStyle = "rgba(255,255,255,0.7)";
            ctx.font = "36px 'PingFang SC','Hiragino Sans GB','Noto Serif SC',serif";
            ctx.fillText("◈ 仓城 · 古韵新生 ◈", W / 2, H * 0.92);
            ctx.font = "28px 'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif";
            ctx.fillText("东南大学暑期社会实践", W / 2, H * 0.96);
...
function bindPosterEvents() {
    ...
    app.onclick = function (e) {
        ...
        // 景点卡片选择
        if (e.target.closest(".poster-spot-card")) {
            var card = e.target.closest(".poster-spot-card");
            var allCards = document.querySelectorAll(".poster-spot-card");
            allCards.forEach(function (c) { c.classList.remove("selected"); });
            card.classList.add("selected");
            posterState.selectedSpot = card.dataset.spot;
            updatePosterButton();
            return;
        }
        // 风格卡片选择
